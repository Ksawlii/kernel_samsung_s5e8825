...
int __generate_irc_v1(struct brightness_table *brt_tbl, struct panel_irc_info *info, u64 luminance, int brightness)
{
	...
	u64 coef = usdm_disp_pow(10, SCALEUP_5);
	int normal_max = lum_tbl[size_ui_lum - 1];
	u64 scaleup_normal_max = normal_max * usdm_disp_pow(10, SCALEUP_4);
	...
	for (gray = 0; gray < 3; gray++) {
		for (color = 0; color < 3; color++) {
			...
			temp_val = usdm_disp_div64(temp_val * luminance, lum_tbl[size_ui_lum - 1]);
			temp_val *= coef;
			info->buffer[dynamic_offset + gray * 3 + color] = (u8)(disp_pow_round(temp_val, 9) / usdm_disp_pow(10, SCALEUP_9));
			for (i = 0; i < gray; i++)
...
int __generate_irc_v2(struct brightness_table *brt_tbl, struct panel_irc_info *info, u64 luminance, int brightness)
{
	...
	if (is_hbm_zone(brt_tbl, brightness)) {				// is hbm
		...
		temp_val /= usdm_disp_pow(10, SCALEUP_5); // scanel down
		coef = coef - temp_val;
	}
	...
	for (i = info->dynamic_offset; i < info->dynamic_offset + info->dynamic_len; i++) {
		...
		temp_val = (u64)disp_pow_round(usdm_disp_div64(temp_val * luminance, lum_tbl[size_ui_lum - 1]), SCALEUP_9);
		info->buffer[i] = (u8)(temp_val / (u64)usdm_disp_pow(10, SCALEUP_9));
	}
...
u64 get_scaleup_luminance(struct brightness_table *brt_tbl, struct panel_irc_info *info, int brightness)
{
	...
	current_lum = usdm_disp_interpolation64(lower_lum * usdm_disp_pow(10, SCALEUP_4), upper_lum * usdm_disp_pow(10, SCALEUP_4),
